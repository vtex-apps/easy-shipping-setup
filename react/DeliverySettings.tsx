...
const DeliverySettings: FC = () => {
    ...
    refetchQueries: [
      {
        ...
        variables: { carrierId: shippingRate.carrierId, zipCode: '0' },
      },
    ],
  })

  const [createDock] = useMutation(CREATE_DOCK, {
    refetchQueries: [{ query: GET_DOCKS }],
  })
  ...
  const [updateShippingPolicy] = useMutation(UPDATE_SHIPPING_POLICY, {
    refetchQueries: [{ query: GET_SHIPPING_POLICIES }],
  })
  ...
  const { data: dockData, loading: dockLoading } = useQuery(GET_DOCKS, {
    ssr: false,
  })
  const {
    data: shippingPolicyData,
    error,
    loading: shippingPolicyLoading,
  } = useQuery(GET_SHIPPING_POLICIES, { ssr: false })
  const { data: allPromotionsData, loading: allPromotionsLoading } = useQuery(
    GET_PROMOTIONS,
    { ssr: false }
  )
  ...
  const [
    getShippingRates,
    { data: shippingRateData, loading: shippingRateLoading },
  ] = useLazyQuery(GET_SHIPPING_RATES)
  const [getPromotionData, { data: promotionData, loading: promotionLoading }] =
    useLazyQuery(GET_PROMOTION_BY_ID)
    ...
    if (!dockLoading && dockData.listAllDocks.length === 0) {
      createDock().then((result) => {
        if (result.data?.createDock === true) {
          if (
            !warehouseLoading &&
            warehouseData?.listAllWarehouses?.length === 0
          )
            createWarehouse()
        }
      })
  ...
  useEffect(() => {
    if (allPromotionsData?.listAllPromotions?.items?.length) {
      filteredPromotion = allPromotionsData.listAllPromotions.items.filter(
        (item: any) => item.name === 'Free delivery'
      )
      setIdCalculatorConfiguration(
        filteredPromotion[0].idCalculatorConfiguration
      )
    }
  }, [allPromotionsData])
  ...
  useEffect(() => {
    if (!shippingPolicyData?.listAllShippingPolicies?.items.length || error)
      return
  ...
  useEffect(() => {
    ...
    setShippingRate({
      ...
      searchZipCode: '0',
      input: shippingRateData.listShippingRate.map(
        (rate: ShippingRateInputData) => ({
  ...
  const handleChangeDelivery = (e: any) => {
    shippingRate.input[0].timeCost =
      e.target.value.length <= 4 ? e.target.value + '.00:00:00' : e.target.value
        ...
        pageHeader={
          <PageHeader
            title={intl.formatMessage({
              id: 'admin/delivery-settings.navigation.label',
            })}
          />
        }
      >
          ...
          {isLoading ? (
            ...
            <div>
              <div className={`flex justify-between`}>
                    ...
                    <div className={`mb4`}>
                      <Input
                        ...
                        label={intl.formatMessage({
                          id: 'admin/shipping-rate.delivery-cost',
                        })}
                      />
                    </div>
                    <div className={`mb4`}>
                      <Input
                        ...
                        label={intl.formatMessage({
                          id: 'admin/shipping-rate.delivery-time',
                        })}
                      />
                    </div>
                  </div>
                ...
                >
                  <div style={{ lineHeight: 2 }} className={`pa7`}>
                    <div>
                      {intl.formatMessage({ id: 'admin/shipping-policy.name' })}{' '}
                      <strong>{shippingPolicy.name}</strong>
                    </div>
                    <div>
                      {intl.formatMessage({ id: 'admin/dock.name' })}
                      <strong>
                        {dockData.listAllDocks.length !== 0
                          ? dockData.listAllDocks[0].name
                          : ''}
                      </strong>
                    </div>
                    <div>
                      {intl.formatMessage({ id: 'admin/warehouse.name' })}
                      <strong>
                        {warehouseData.listAllWarehouses.length !== 0
                          ? warehouseData.listAllWarehouses[0].name
                          : ''}
                      </strong>
                    </div>
                  </div>
              ...
              {Object.keys(shippingPolicy).length !== 0 && (
                <div>
                  ...
                  <div className={`flex mb4`}>
                    <Toggle
                      label={intl.formatMessage({
                        id: 'admin/shipping-policy.label.holiday',
                      })}
                  ...
                  <div className={`flex mb4`}>
                    <Toggle
                      label={intl.formatMessage({
                        id: 'admin/shipping-policy.label.saturday',
                      })}
                  ...
                  <div className={`flex mb4`}>
                    <Toggle
                      label={intl.formatMessage({
                        id: 'admin/shipping-policy.label.sunday',
                      })}
              ...
              <div className={`flex mb6`}>
                <Toggle
                  label={intl.formatMessage({
                    id: 'admin/promotion.label.isActive',
                  })}
                  checked={isActive}
              ...
              {isActive && (
                <div className={`w-40`}>
                  <Input
                    ...
                    label={intl.formatMessage({
                      id: 'admin/promotion.label.free-delivery-threshold',
                    })}
              ...
              <ToastConsumer>
                {({ showToast }: { showToast: any }) => (
                  <div className={`pt6`}>
                    ...
                    >
                      {intl.formatMessage({
                        id: 'admin/shipping-policy.button.update',
                      })}
                    </Button>
                  </div>
                )}
